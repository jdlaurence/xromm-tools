...
Now let's fo an example of how to automate this across multiple trials. Let's first duplicate the files with new names so we have some data to work with. NOTE-- you will be copying files directly to your project directory, so make sure you replace these variables as needed! (This will also let you practice using local files instead of ones built in to the package.)
...
for (tr in c(1:n_trials)){
  ...
  rbt_filename <- paste0(paste0("../../../xromm-batch-testing/rbt_",
                                trialnames[tr]), ".csv")
  xyz_filename <- paste0(paste0("../../../xromm-batch-testing/3dpts_", 
                                trialnames[tr]), ".csv")
